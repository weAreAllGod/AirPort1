...
if __name__ == '__main__':
    ...
    for j in range(lenInitalData):
        ...
        thisNationOfGate = 1 if allGate[allGate["gateno"] == str(initalDataParkingGate[j])]["nation"].iloc()[
                                    0] == "国际" else 0
    ...
    dataWrong = initalData.loc[
        (initalData["parkinggate"].isin(nationalGateInf["gateno"].to_list())) & (initalData["nation"] == 1)]
    ...
    # 第二种贪心分配方法
    print("------------------------贪心法计算结果--------------------->")
    gatePlanDict2 = funcForColumn.gateToPlanWithInitialStata(initalData, allGate, result, resultInf, atimList, dtimList,
                                                             nationOfResult, typeOfResult)
