...
if __name__ == '__main__':
    ...
    #总的数据
    # initalData = initalData.sort_values(by="atime")
    ...
    initalData=mydata.iloc[:len(initalData),:]
    ...
    initalDataParkingGate=initalData["parkinggate"].to_list()
    ...
    for j in range(lenInitalData):
        thisTypeOfGate=getTypeOfPosition(allGate[allGate["gateno"] == str(initalDataParkingGate[j])]["mdl"].iloc()[0])
        thisNationOfGate=1 if allGate[allGate["gateno"] == str(initalDataParkingGate[j])]["nation"].iloc()[0]=="国际" else 0
        for jj in range(reMatrix.shape[1]) :
            #从第8行开始写,这里注意机位类型以及国内国外的约束
            if j in possibles[jj] :
                reMatrix[8 + j, jj] = 1
                # if len(possibles[jj]) > 1:
                #     behindPossibles = possibles[jj][1:]
                #     if max([nationOfPlan[item] for item in behindPossibles])<=thisTypeOfGate and max([getTypeOfPosition(typeOfplan[item]) for item in behindPossibles])<=thisTypeOfGate:
                #         reMatrix[8 + j, jj] = 1
                # else:
                #     reMatrix[8+j,jj]=1
    # 后面飞机的约束
    for i1 in range(lenInitalData,len(atimList)):
        for ii in range(reMatrix.shape[1]):
            if i1 in possibles[ii]:
                reMatrix[i1+8, ii] = 1


    # 对于初始状态没有按照规定进行分配的进行特殊的处理
    dataWrong=initalData.loc[(initalData["parkinggate"].isin(nationalGateInf["gateno"].to_list())) &(initalData["nation"]==1)]
    ...
    print("矩阵数据准备完毕,下面开始求解", datetime.datetime.now() - beginTime)
    ...
    #第二种贪心分配方法
    print("------------------------贪心法计算结果--------------------->")
    gatePlanDict2=funcForColumn.gateToPlanWithInitialStata(initalData,allGate,result,resultInf,atimList,dtimList,nationOfResult,typeOfResult)
