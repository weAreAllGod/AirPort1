...
###3.初始化航班和机位数据
assignment = open("0602ad-csv.csv", 'r')
...
def gate_assignment_model(flightlist,gatelist):
    ...
    m = Model("gateassign")
    # 建立决策变量
    x = m.addVars(Z.keys(), vtype=GRB.BINARY,
                  name='x')
    # 添加各项约束
    ## 匹配性约束：针对Q，Z矩阵
    for i in range(len(flightlist)):
    ...
    ##每个航班最多分配一个机位（约束放松）
    for i in range(len(flightlist)):
        m.addConstr(quicksum(x[i, k] for k in range(len(gatelist))) <= 1)
    ...
    for i in range(len(flightlist)):
        for j in range(len(gatelist)):
            if solution[i, j] == 1:
                d[i, j] = 1
                ##若航班i被分配至机位j，则更新航班i的gate属性，并将航班并入已分配航班集合
                flightlist[i].gate=gatelist[j].gateno
...
def gate_assignment_model_strict(flightlist, gatelist):
    ...
    # 建立目标函数
    m.setObjective(quicksum(x[i, k] * int(gatelist[k].bridge) * gategravity[gatelist[k].gateno[0]] for i, k in arcs),
                   GRB.MAXIMIZE)
    m.optimize()
    d = {}
    assigned_flightlist = []
...
for i in range(len(list_flt)):
    for index in range(2):
        ...
        if len(list_flt[i][index])>0:
            ...
            ##利用函数进行分配
            if len(gt_sum)>0:
                print('求解规模为：',len(list_flt[i][index]),len(gt_sum))
...
for i in range(len(list_flt)):#0~3
    for index in range(2):  #0、1
        ...
        if len(noassign_flt) > 0:
            ...
            if len(gt_sum)>0:
                print('求解规模为：',len(noassign_flt),len(gt_sum))
